...
if __name__ == '__main__':
  ...
  def calculate_train_test_scores(seed, swap_train_test, model_unchanged, out_dir):
    ##
    # Step 1: Train the model
    ##

    ans = train_test_split(INP, OBS, OBS2, NAMES, DEL_LENS, test_size = 0.5, random_state = seed)
    INP_train, INP_test, OBS_train, OBS_test, OBS2_train, OBS2_test, NAMES_train, NAMES_test, DEL_LENS_train, DEL_LENS_test = ans

    if swap_train_test:
       INP_train, INP_test = INP_test, INP_train
       OBS_train, OBS_test = OBS_test, OBS_train
       NAMES_train, NAMES_test = NAMES_test, NAMES_train
       OBS2_train, OBS2_test = OBS2_test, OBS2_train
       DEL_LENS_train, DEL_LENS_test = DEL_LENS_test, DEL_LENS_train
    ...
    def print_perf(nn_params, nn2_params, iter):
        log_out_path = f"{out_dir}/_log_default_model.out" if model_unchanged else f"{out_dir}/_log_modified_model.out"
        ...
        if iter % 50 == 0:
            params_out_dir = f"{out_dir}/params-default-model/" if model_unchanged else f"{out_dir}/params-modified-model/"
            util.ensure_dir_exists(params_out_dir)
            ...
            ut.save_parameters(nn_params, nn2_params, params_out_dir, f"iter_{iter}")

    ##
    # Step 2: Calculate scores (currently just test score)
    ##
    
    nn_params, nn2_params = bp.adam_minmin(both_objective_grad, init_nn_params, init_nn2_params, step_size = step_size, num_iters = num_epochs, callback = print_perf)
    test_score = main_objective(nn_params, nn2_params, INP_test, OBS_test, OBS2_train, DEL_LENS_test, len(INP_test), seed)
